...
import { adminProfile, deleteDoctorQuestion, deleteQuestion } from '@/hooks/ReactQueryHooks';
...
export default function ModalDeleteQuestion({ selectedQuestion, showModalDelete, setShowModalDelete }) {
    ...
    const { mutate: deleteQues } = useMutation({
        mutationFn: ({ role, id }) => deleteDoctorQuestion({ role, id }),
        onSuccess: (data) => {
    ...
    return (
        <div className="flex items-center justify-center bg-gray-100">
            <ConfirmModal
                ...
                iconColor="#7B1E19"
                onConfirm={() => deleteQues({ role: profile?.role, id: selectedQuestion?.id })}
            />
        </div>
    )
}
